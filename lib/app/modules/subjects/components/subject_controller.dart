...
class SubjectController extends ChangeNotifier {
  ...
  Future<void> getSubjectList() async {
    try {
      loading.show();
      final response = await apiService.getList<SubjectModel>(
        AppUrs.subjectsAll(),
  ...
  Future<void> getSubjectDetailsById(String id) async {
    try {
      loading.show();
      final response = await apiService.get<SubjectDetails>(
        AppUrs.subjectDetails(id),
        SubjectDetails.fromJson,
      );
      if (response.success) {
  ...
  Future<VideoModel?> getVideoById(String? id) async {
    try {
      loading.show();
      final response = await apiService.get<VideoModel>(
        AppUrs.video(id),
        VideoModel.fromJson,
      );
      if (response.success) {
  ...
  Future<void> getAllVideosBySubjectId(String? id) async {
    try {
      loading.show();
      final response = await apiService.getList<VideoModel>(
        AppUrs.videoAllVideoSubjectId(id),
        VideoModel.fromJson,
      );
      if (response.success) {
  ...
  Future<bool?> createTest(Map<String, dynamic> data) async {
    try {
      loading.show();
      final response = await apiService.post<TestItem>(
        endpoint: AppUrs.test,
        data: data,
        fromJsonT: TestItem.fromJson,
      );
      if (response.success) {
        if (response.data!= null) {
          details?.tests.add(response.data!);
        }
        notifyListeners();
      } else {
